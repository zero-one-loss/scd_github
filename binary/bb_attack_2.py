...
if args.dataset == 'mnist':
    image_shape = 28 * 28
elif args.dataset == 'cifar10' or args.dataset == 'cifar10_binary':
    image_shape = 32 * 32 * 3
...
class Oracle(object):
    ...
    def predict(self, x, batch_size):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            preds.append(torch.from_numpy(self.svc.predict(data.numpy().transpose((0, 2, 3, 1)).reshape((data.shape[0], -1)))).long())
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(self.data_loader):
                ...
                preds.append(torch.from_numpy(
                    self.svc.predict(data.numpy().transpose((0, 2, 3, 1)).reshape((data.shape[0], -1)))).long())
